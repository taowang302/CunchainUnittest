...
class  RunCase:
    ...
    def run_case(self, runner, run_mode, run_case_list, db_conn, http, log, archive_id):
        ...
        if int(archive_num) != 1:
            ...
            db1_cursor.execute("UPDATE test_result set actual_response=NULL ,result=NULL,description=Null,actual_response_code=Null where from_view_id='{}'".format(archive_id))
        ...
        if 1 == run_mode:
            ...
            for case_id in range(1, test_case_num+1):
                db1_cursor = db_conn.cursor()
                 ...
                 test_suite.addTest(TestInterfaceCase(test_data.test_method, test_data, http, db1_cursor, log=log, archive_id=archive_id))
