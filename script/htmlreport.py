...
class HtmlReport:
    ...
    def generate_html(self,head, file):
            ...
            # 查询测试失败的用例数
            self.cursor.execute('SELECT count(case_number) FROM test_result WHERE result = %s',('Fail',))
            self.fail_num = self.cursor.fetchone()[0]

            # 查询测试通过的用例数
            self.cursor.execute('SELECT count(case_number) FROM test_result WHERE result = %s',('Pass',))
            self.success_num = self.cursor.fetchone()[0]

            # 查询测试出错的用例数
            self.cursor.execute('SELECT count(case_number) FROM test_result WHERE result = %s',('Error',))
